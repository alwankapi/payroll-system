...
class UpdatePenggajianRequest extends FormRequest
{
    ...
    {
        // BR-01: Hanya Admin yang dapat update data Penggajian
    ...
    public function rules(): array
    {
        return [
            ...
            'periode' => [
                ...
                'date',
                'date_format:Y-m',
                // BR-05: Validasi unique kombinasi karyawan_id + periode kecuali record saat ini
                Rule::unique('penggajians')->where(function ($query) {
                    return $query->where('karyawan_id', $this->karyawan_id);
                })->ignore($this->route('penggajian')),
            ],
            'gaji_pokok' => [
                'required',
                'numeric',
