    ...
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6">
            <div class="flex items-center justify-between mb-6">
                <h2 class="text-xl font-semibold text-gray-900 dark:text-white">Edit Penggajian</h2>
                <a href="{{ route('penggajians.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-300 dark:bg-gray-600 border border-transparent rounded-md font-semibold text-xs text-gray-700 dark:text-gray-200 uppercase tracking-widest hover:bg-gray-400 dark:hover:bg-gray-500 transition">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
                    Kembali
                </a>
            </div>

            <form action="{{ route('penggajians.update', $penggajian) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="month" name="periode" id="periode" value="{{ old('periode', $penggajian->periode) }}" required class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm @error('periode') border-red-500 @enderror">
                    ...
                    <div>
                        ...
                        <input type="number" name="gaji_pokok" id="gaji_pokok" value="{{ old('gaji_pokok', $penggajian->gaji_pokok) }}" required min="0" step="1" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm @error('gaji_pokok') border-red-500 @enderror">
                    ...
                    <div>
                        ...
                        <input type="number" name="tunjangan" id="tunjangan" value="{{ old('tunjangan', $penggajian->tunjangan) }}" required min="0" step="1" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm @error('tunjangan') border-red-500 @enderror">
                ...
                <div>
                    ...
                    <div class="space-y-2 border border-gray-300 dark:border-gray-700 rounded-md p-4">
                        @php
                            $selectedPotonganIds = old('potongan_ids', $penggajian->potongans->pluck('id')->toArray());
                        @endphp
                        @foreach($potongans as $potongan)
                            <label class="flex items-center">
                                <input type="checkbox" name="potongan_ids[]" value="{{ $potongan->id }}" 
                                    {{ is_array($selectedPotonganIds) && in_array($potongan->id, $selectedPotonganIds) ? 'checked' : '' }}
                ...
                <div class="flex items-center justify-end gap-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                    <a href="{{ route('penggajians.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-300 dark:bg-gray-600 border border-transparent rounded-md font-semibold text-xs text-gray-700 dark:text-gray-200 uppercase tracking-widest hover:bg-gray-400 dark:hover:bg-gray-500 transition">Batal</a>
