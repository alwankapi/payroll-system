            ...
            <form action="{{ route('potongan.store') }}" method="POST" class="space-y-6" x-data="potonganForm()">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="nama_potongan" id="nama_potongan" value="{{ old('nama_potongan') }}" required class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm @error('nama_potongan') border-red-500 @enderror">
                    ...
                    <div>
                        ...
                        <select name="jenis_potongan" id="jenis_potongan" required x-model="jenis" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm @error('jenis_potongan') border-red-500 @enderror">
                            ...
                            <option value="persentase" {{ old('jenis_potongan') === 'persentase' ? 'selected' : '' }}>Persentase (%)</option>
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="nilai" id="nilai" value="{{ old('nilai') }}" required step="0.01" min="0" x-on:input="calculatePreview" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm @error('nilai') border-red-500 @enderror">
                        @error('nilai')<p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>@enderror
                    </div>

                    <div class="md:col-span-2">
                        <label for="deskripsi" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Deskripsi</label>
                        <textarea name="deskripsi" id="deskripsi" rows="3" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm">{{ old('deskripsi') }}</textarea>
                    </div>

                    <div>
                        <label class="flex items-center">
                            <input type="checkbox" name="status_aktif" value="1" {{ old('status_aktif', true) ? 'checked' : '' }} class="rounded border-gray-300 dark:border-gray-700 dark:bg-gray-900 text-indigo-600 focus:ring-indigo-500">
